...
class TileManager:
    ...
    @staticmethod
    def read_tiles_from_dir(
        idp, cover=None, target_raster_name=None, is_dataset_dp=True
    ):
        """Loads files from an on-disk dir."""
        idp = os.path.expanduser(idp)
        tile_relative_fp_scheme = (
            TilePathManager.read_relative_tile_fp_scheme_from_dir(
                idp, is_dataset_dp
            )
        )
        ...
        for tile_ifp in tile_ifp_list:
            if tile_ifp.endswith("aux.xml"):
                continue
            tile = TilePathManager.convert_tile_fp_to_tile(
                idp, tile_ifp, is_dataset_dp
            )
