...
class ImagePixelTile(Tile):
    ...
    def get_source_rectangle(self):
        ...
        return x_offset, y_offset, x_offset + width - 1, y_offset + height - 1
    ...
    def write_pixel_corners_as_geojson(self, geojson_ofp, as_polygon=False):
        disk_width, disk_height = self.get_disk_size()

        # Option 1:
        # raster_transform = self.get_raster_transform()
        # source_x_offset, source_y_offset = self.get_source_offset()
        # ul_pix = (source_x_offset + 0.5, source_y_offset + 0.5)
        # ur_pix = (source_x_offset + disk_width + 0.5, source_y_offset + 0.5)
        # lr_pix = (source_x_offset + disk_width + 0.5, source_y_offset + disk_height + 0.5)
        # ll_pix = (source_x_offset + 0.5, source_y_offset + disk_height + 0.5)
        # ul_coord = raster_transform * ul_pix
        # ur_coord = raster_transform * ur_pix
        # lr_coord = raster_transform * lr_pix
        # ll_coord = raster_transform * ll_pix

        # Option 2:
        tile_transform = self.get_tile_transform()
        ul_coord = tile_transform * (0.5, 0.5)
        ur_coord = tile_transform * (disk_width + 0.5, 0.5)
        lr_coord = tile_transform * (disk_width + 0.5, disk_height + 0.5)
        ll_coord = tile_transform * (0.5, disk_height + 0.5)

        corner_dst_crs_list = [ul_coord, ur_coord, lr_coord, ll_coord]

        # Geojson coordinates must be in EPSG_4326
        if self.get_crs() is not None:
            corner_epsg_4326_list = _transform_coord_list(
                corner_dst_crs_list, self.get_crs(), EPSG_4326
            )
        else:
            # If no crs is defined, we return the coordinates "as is".
            # In this case QGIS uses automatically EPSG 4326 to show the
