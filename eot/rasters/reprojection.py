...
from eot.rasters.geo import get_default_transformation
from eot.rasters.src import get_src_raster
from eot.rasters.dst import get_dst_driver
...
def get_default_transform_parameter(ifp_or_src, dst_crs=None):
    with get_src_raster(ifp_or_src) as src:
        if dst_crs is None:
            dst_crs = src.crs
        default_transform, width, height = get_default_transformation(
            src, dst_crs
        )
    return default_transform, dst_crs, width, height


def reproject_dataset_with_default_transform(
...
@contextmanager
def get_reprojected_dataset_generator(
    ifp_or_src,
    dst_crs,
    dst_transform,
    dst_width,
...
def get_reprojected_dataset_with_default_transform_generator(
    ifp_or_src, dst_crs=None, resampling=Resampling.nearest
):
    default_transform, width, height = get_default_transformation(
        ifp_or_src, dst_crs
    )
    dst_dataset_generator = get_reprojected_dataset_generator(
        ifp_or_src,
        dst_crs=dst_crs,
