...
from eot.rasters.write import (
    write_numpy_as_dataset,
    write_dataset,
    get_written_dataset_generator,
)
...
from eot.utility.np_utility import get_non_black_pixel_indices
...
def _print_aggregation_msg(requested_category_titles, idp, ofp):
    print(
        f"neo aggregate {requested_category_titles} from {idp} to {ofp}",
        file=sys.stderr,
        flush=True,
    )
    ...
    **kwargs,
    ...
    if original_raster is None:
        ...
        if tile_class == MercatorTile:
            with get_written_dataset_generator(
                tiling_raster,
                ...
                **kwargs,
                ...
                transform, crs = original_raster.get_transform_with_crs()
                assert resampling is not None
                reproject_dataset(
...
def create_images_with_pixel_projection(args, masks, resampling):
...
def _get_target_tile_raster_area(tile, tiling_raster):
    if isinstance(tile, MercatorTile):
        left, bottom, right, top = tile.get_bounds_src(tiling_raster.get_crs())
...
def create_images_from_tiles_with_pixel_projection(
    args, masks, tile_class, resampling
):
    ...
    with _get_tiling_raster(
        ...
        mask_values = compute_mask_values(args.mask_values, category_indices)
        for tile in tqdm(masks, ascii=True, unit="mask"):
            tile_label_mat, _ = get_tile_label_mat(tile.get_absolute_tile_fp())
            ...
            tile_label_mat = cv2.resize(
                tile_label_mat,
                tile_size,
                interpolation=label_mask_resampling_cv,
            )
            assert tile_size == tile_label_mat.shape[-2:]
            ...
            for idx, category_index in enumerate(category_indices):
                category_color = args.idx_to_category_color[category_index]
                category_color_opaque = (*category_color, 255)
                category_color_alpha = (*category_color, args.overlay_weight)
                category_value = mask_values[idx]

                tile_mask = get_tile_mask(tile_label_mat, category_index)
                ...
                raster_mask[r_cat_y_pix, r_cat_x_pix] = category_value
                raster_mask_color[
        ...
        if args.gray_mask_png_ofp is not None:
            _print_aggregation_msg(
                args.requested_category_titles,
                args.masks_idp,
